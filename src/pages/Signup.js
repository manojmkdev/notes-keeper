...
export default function Signup() {
  ...
  const handleSubmit = (e) => {
    ...
    const existing = JSON.parse(localStorage.getItem('nk_users') || '[]');
    if (existing.find(u => u.email === form.email)) {
      toast.error('An account with this email already exists.');
      return;
    }

    const user = { name: form.name, email: form.email, password: form.password, memberSince: new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) };
    existing.push(user);
    localStorage.setItem('nk_users', JSON.stringify(existing));
      ...
      <div className="auth-right">
        <div className="auth-card">
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <button type="submit" className="auth-btn w-100">Create Account</button>
          </form>
