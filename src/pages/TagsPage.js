...
import { getTags, saveTags, getNotes, saveNotes, getCurrentUser } from '../utils/Storage';
...
export default function TagsPage() {
  ...
  const loadData = () => {
    ...
    setTags(allTags.filter((t) => !t.userId || t.userId === currentUser.email));
  ...
  const handleSaveEdit = (id) => {
    ...
    const exists = tags.some(
      (t) => t.id !== id && t.name.toLowerCase() === editingName.trim().toLowerCase()
    );
    if (exists) {
      toast.error('A tag with this name already exists.');
      return;
    ...
    const updated = allTags.map((t) =>
      t.id === id ? { ...t, name: editingName.trim(), color: editingColor } : t
    );
    saveTags(updated);
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this tag? It will be removed from all your notes.')) {
      const allTags = getTags();
      const updatedTags = allTags.filter((t) => t.id !== id);
      saveTags(updatedTags);

      // Remove tag from ALL notes in storage
      const allNotes = getNotes();
