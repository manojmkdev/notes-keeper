...
import { getNotebooks, saveNotebooks, getNotes, saveNotes, getCurrentUser } from '../utils/Storage';
...
export default function NotebooksPage() {
  ...
  const loadData = () => {
    ...
    setNotebooks(allNotebooks.filter((nb) => !nb.userId || nb.userId === currentUser.email));
    setNotes(allNotes.filter((n) => !n.userId || n.userId === currentUser.email));
  };
  ...
  const handleCreate = (e) => {
    ...
    const exists = notebooks.some(
      (nb) => nb.name.toLowerCase() === newNotebookName.trim().toLowerCase()
    );
    if (exists) {
    ...
    const allNotebooks = getNotebooks();
    const updated = [newNb, ...allNotebooks];
  ...
  const handleSaveEdit = (id) => {
    ...
    const exists = notebooks.some(
      (nb) => nb.id !== id && nb.name.toLowerCase() === editingName.trim().toLowerCase()
    );
    if (exists) {
      toast.error('A notebook with this name already exists.');
      return;
    ...
    const allNotebooks = getNotebooks();
    const updated = allNotebooks.map((nb) =>
      nb.id === id ? { ...nb, name: editingName.trim() } : nb
    );
    saveNotebooks(updated);
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this notebook? Notes in this notebook will not be deleted but will be uncategorized.')) {
      const allNotebooks = getNotebooks();
      const updatedNb = allNotebooks.filter((nb) => nb.id !== id);
      saveNotebooks(updatedNb);

      // Unassign this notebook from ALL notes in storage
      const allNotes = getNotes();
